...
public class HBaseClientOperations {
    ...
    private void put(Table table) throws IOException {
        ...
        for (String[] user : users) {
            ...
            put.addImmutable(Bytes.toBytes("name"), Bytes.toBytes("first"), Bytes.toBytes(user[1]));
            put.addImmutable(Bytes.toBytes("name"), Bytes.toBytes("last"), Bytes.toBytes(user[2]));
            put.addImmutable(Bytes.toBytes("contactInfo"), Bytes.toBytes("email"), Bytes.toBytes(user[3]));
            table.put(put);
        }
    }
     ...
     * @throws IOException Error occurred when get data from the table.
     */
    private void get(Table table, byte[] row, byte[] family, byte[] qualifier) throws IOException {
     ...
     * @param family    The family.
     * @param qualifier The qualifier.
     * @throws IOException Error occurred when scanner from the table.
     */
    private void scan(Table table, byte[] family, byte[] qualifier) throws IOException {
     ...
     * @throws IOException Error occurred when filter from the table.
     */
    private void filters(Table table, byte[] family, byte[] qualifier, String value) throws IOException {
